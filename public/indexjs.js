...
// firebase sdk
  ...
  getDocs(productQuery).then((결과) => {
    결과.docs.forEach((doc) => {
      ...
      // 좋아요 정보 가져오기
      let likes = Array.isArray(imageData.wholike) ? imageData.wholike : [];

      // wholike 기능 관련 변수 초기화 for 그냥 view 유저들
      let isLiked = false;
    ...
    $(document).on("click", ".like-icon", async function () {
      ...
      const likes = postData.wholike || [];

      if (likes.includes(user.uid)) {
        // 이미 좋아요를 눌렀으면 좋아요 취소
        const newLikes = likes.filter((uid) => uid !== user.uid);
        await updateDoc(postRef, { wholike: newLikes, wholikeCount: newLikes.length });
